...
python -u -m torch.distributed.run --nproc_per_node=2 --rdzv_endpoint $(scontrol show hostnames $SLURM_JOB_NODELIST | head -n 1):6000:6000  --rdzv_backend c10d \
all_reduce_bench.py
...
def run(local_rank, size):

    start_time = time.time() # Starting time

    hostname = socket.gethostname() # Get the hostname
    is_global_rank_0 = dist.get_rank() == 0 # Get the global rank
    ...
    def finish():
        ...
        print(f"busbw: {conv_to_GBps(busbw):6.2f}GBps")
        print(f"algbw: {conv_to_GBps(algbw):6.2f}GBps")
    ...
    algbw = torch.mean(torch.stack(algbw_gather)).item()
...
def init_processes(local_rank, fn, backend='nccl', size=15):
...
size = int(sys.argv[1])
size = 2**size
init_processes(local_rank=local_rank, fn=run, size=size)
