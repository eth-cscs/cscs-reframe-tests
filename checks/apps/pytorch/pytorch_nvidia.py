...
class PyTorchDdpMambaNv(PyTorchTestBase):
    ...
    def activate_venv(self):
        self.prerun_cmds = [
            ...
            'bash Miniforge3-$(uname)-$(uname -m).sh -b -p ./mamba',
            '. mamba/etc/profile.d/conda.sh',
            # '. mamba/etc/profile.d/mamba.sh',
            f'conda create -p ./mamba/envs/rfm {self.torch_version} -c pytorch -c nvidia -y',
            'conda activate ./mamba/envs/rfm',
