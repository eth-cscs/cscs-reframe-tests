...
install_reframe() {
    ...
    unzip -qq "sanity_logging.zip" && cd reframe-sanity_logging && ./bootstrap.sh &> /dev/null)
    export PATH="$(pwd)/reframe-sanity_logging/bin:$PATH"
    echo "$(pwd)/reframe-sanity_logging/bin"
